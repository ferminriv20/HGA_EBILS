...
def read_pmed(ruta_archivo: str) -> np.ndarray:
    """
    Lee una instancia del pMP en formato OR-Library 
    y devuelve la matriz de costos completa. 
    
    Args:
        ruta_archivo (str): Ruta al archivo de texto con la instancia pMP.
    
    Returns:
        np.ndarray: Matriz de costos completa (n x n).
...
def generar_datasets_pmedian(carpeta_entrada: str = "pmed", carpeta_salida: str = "datasets") -> None:
    ...
    Args:
        carpeta_entrada (str): Carpeta con archivos .txt de instancias pMP.
        carpeta_salida (str): Carpeta donde se guardarán los archivos .npy.
